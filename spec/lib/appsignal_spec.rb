describe Appsignal do
  ...
  context "with config and started" do
    ...
    describe ".add_query_parameters" do
      ...
      describe "setting the query parameters through the public API" do
        let(:transaction) { http_request_transaction }

        def perform
          set_current_transaction(transaction)
          Appsignal.add_query_parameters("param1" => "value1")
        end
      ...
      context "without transaction" do
        it "does not set the query parameters on any transaction" do
          expect_any_instance_of(Appsignal::Transaction).to_not receive(:add_query_parameters)

          Appsignal.add_query_parameters("a" => "b")
        end
      end
    end
