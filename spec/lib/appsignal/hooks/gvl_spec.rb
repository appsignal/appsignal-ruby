describe Appsignal::Hooks::GvlHook do
  ...
  def allow_gvltools_require
    allow_any_instance_of(described_class).to receive(:require).with("gvltools").and_return(true)
  end
  ...
  context "with new versions of GVLTools" do
    ...
    before(:each) { allow_gvltools_require }
    ...
    describe "#install" do
      ...
      context "without enable_gvl_waiting_threads" do
        it "does not enable the GVL waiting_threads" do
