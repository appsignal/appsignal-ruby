...
module ModeHelpers
  ...
  # agent in the body (`:manual_start`, opting out of the mode contexts'
  # automatic start): agent mode via `start_agent`, collector mode via
  ...
  def it_in_both_modes(description = nil, &block)
    it([description, "in agent mode"].compact.join(" "), :agent_mode, :manual_start) do
      start_agent(**(defined?(start_agent_args) ? start_agent_args : {}))
      instance_exec(&block)
    end
    it([description, "in collector mode"].compact.join(" "), :collector_mode, :manual_start) do
