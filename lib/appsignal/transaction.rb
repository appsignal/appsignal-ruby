...
module Appsignal
  class Transaction
    ...
    def initialize( # rubocop:disable Metrics/ParameterLists
      ...
      # `:request_payload`, `:function_parameters`, `:query_parameters`) to a
      # storage bucket. The extension backend maps them all to a single
      # `:params` bucket, so agent mode keeps one merged params blob. The
      # OpenTelemetry backend maps the request payload, function parameters and
      # query parameters to separate buckets, so collector mode keeps them apart
      # as distinct attributes. One `SampleData` object is allocated per
      # distinct bucket.
